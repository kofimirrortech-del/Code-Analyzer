...
const ALL_ROLES = Object.values(AuthUserRole) as string[];

export const NAV_ITEMS = [
  { path: "/", label: "Dashboard", icon: LayoutGrid, roles: ALL_ROLES },
  ...
  { path: "/history", label: "History", icon: HistoryIcon, roles: ALL_ROLES },
];
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background flex w-full overflow-hidden selection:bg-primary/30">
      <aside className="hidden lg:flex w-64 flex-col glass-panel border-y-0 border-l-0 rounded-none z-20">
        <SidebarContent />
      </aside>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <motion.aside initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }}
            ...
            className="absolute top-0 left-0 bottom-0 w-64 glass-panel border-y-0 border-l-0 rounded-none flex flex-col">
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden pt-16 lg:pt-0 relative">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 relative z-10">
          <motion.div key={location} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, ease: "easeOut" }} className="max-w-7xl mx-auto">
            {children}
          </motion.div>
        </div>
      </main>
