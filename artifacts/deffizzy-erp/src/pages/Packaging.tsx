...
export default function Packaging() {
  ...
  const openModal = (item?: Package) => {
    if (item) {
      setEditingItem(item);
      reset({ packageType: item.packageType, stock: item.stock, expiryDate: item.expiryDate.split('T')[0] });
    } else {
      setEditingItem(null);
      reset({ packageType: "", stock: 0, expiryDate: new Date().toISOString().split('T')[0] });
    }
    setIsModalOpen(true);
  };
  ...
  const onSubmit = (data: FormValues) => {
    if (editingItem) {
      updateMutation.mutate({ id: editingItem.id, data: { ...data, expiryDate: new Date(data.expiryDate).toISOString() } });
    } else {
      createMutation.mutate({ data: { ...data, expiryDate: new Date(data.expiryDate).toISOString() } });
    }
  };

  const handleDelete = (id: number) => {
    if (confirm("Are you sure you want to delete this packaging record?")) {
  ...
  return (
    <div className="space-y-6 pb-10">
                ...
                items.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                      ...
                      <td className="p-4">
                        <span className={`px-2 py-1 rounded text-xs font-medium ${item.stock < 50 ? 'bg-red-500/20 text-red-400' : 'bg-white/10 text-white'}`}>
                          {item.stock}
                        </span>
                      </td>
                      <td className="p-4">
      ...
      <Modal isOpen={isModalOpen} onClose={closeModal} title={editingItem ? "Edit Package" : "Add Package"}>
