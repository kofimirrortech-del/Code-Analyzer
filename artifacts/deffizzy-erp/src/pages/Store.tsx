...
export default function Store() {
  ...
  const [editingItem, setEditingItem] = React.useState<StoreItem | null>(null);
  ...
  const openModal = (item?: StoreItem) => {
    if (item) {
      setEditingItem(item);
      reset({ itemName: item.itemName, quantity: item.quantity, supplier: item.supplier });
    } else {
      setEditingItem(null);
      reset({ itemName: "", quantity: 0, supplier: "" });
    }
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            <p className="text-muted-foreground">Manage general tools and equipment.</p>
            ...
            <tbody className="divide-y divide-white/5">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground">No items in store. Add one to get started.</p>
                </td></tr>
              ) : (
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${item.quantity < 10 ? 'bg-red-500/20 text-red-400' : 'bg-white/10 text-white'}`}>
                        {item.quantity} units
                      </span>
                    </td>
      ...
      <Modal 
        ...
        description="Enter the inventory details below."
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
          <div>
            <label className="text-xs font-semibold text-muted-foreground uppercase mb-1 block">Item Name</label>
            <Input {...register("itemName")} placeholder="e.g. Mixing Bowl" error={errors.itemName?.message} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input {...register("supplier")} placeholder="e.g. Kitchen Co" error={errors.supplier?.message} />
            </div>
          </div>
