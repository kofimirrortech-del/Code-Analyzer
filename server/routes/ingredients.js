...
router.get('/', requireAuth, async (req, res) => {
  ...
  if (rows.length === 0 && date === today()) {
    const { rows: latest } = await query(
      `SELECT DISTINCT ON (name) name, closing_stock, low_stock_threshold, unit, supplier
       FROM ingredients WHERE date < $1 ORDER BY name, date DESC, id DESC`,
      [date]
    );
    if (latest.length > 0) {
      for (const r of latest) {
        ...
        await query(
          `INSERT INTO ingredients (name,stock,opening_stock,added_stock,closing_stock,low_stock_threshold,unit,supplier,date)
           VALUES ($1,$2,$2,0,$2,$3,$4,$5,$6)`,
          [r.name, opening, r.low_stock_threshold ?? 0, r.unit ?? 'units', r.supplier ?? '', date]
        );
      }
      const { rows: newRows } = await query('SELECT * FROM ingredients WHERE date=$1 ORDER BY created_at ASC', [date]);
